...
static int run_sql(sqlite3 *db, t_message *message) {
    char *sql = "UPDATE message SET text = '%s' WHERE message_id = %d;";
    int size = mx_strlen(sql);
    char *resolved = mx_strnew(size * 2);
    sprintf(resolved, sql, message->message, message->id);
    ...
    if (rc != SQLITE_OK) {
        ...
        mx_log_i("DB", "Updated message with id '%d' in data base \"%s\"",
